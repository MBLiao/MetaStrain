...
MAX_GENERATIONS = 4  # max generation
CROSSOVER_RATE = 0.8   # cross rate
MUTATION_RATE = 0.04   # mutation rate
# Convergence detection parameters
CONVERGENCE_THRESHOLD = 0.001  # Define the threshold for fitness change
...
if __name__ == '__main__':
    ...
    # wildSolution_moma = wild_glucose_constraint(ecYeast)
    ...
    ts_init = time.time()
    population = ini_rand_operation(POPULATION_SIZE, GENE_LENGTH)
    ...
    for generation in range(MAX_GENERATIONS):
        ...
        for i in range(len(c_pop)):
            mutated_ind, mflag = mutation_rand(c_pop[i], MUTATION_RATE, GENE_LENGTH)
            if mflag == 1:
    ...
    df.to_csv(r"C:\Users\Wenb1n\Desktop\result_check.csv", index=False)
